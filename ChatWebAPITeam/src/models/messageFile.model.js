...
const create = async (reqBody, file) => {
  try {
    const folder = file?.file_type?.includes('application') ?
      CLOUDINARY_FOLDER.MESSAGE.DOCUMENT :
      CLOUDINARY_FOLDER.MESSAGE.MEDIA
    const uploadedFiles = await cloudinaryProvider.uploadSingleFile(file, folder);
    const messageFile = {
      ...reqBody,
      file_url: uploadedFiles
    }
    const { query, values } = generateInsertQuery("message_files", messageFile)
    const [result] = await GET_DB().query(query, values)
    return result.insertId ? { id: result.insertId, ...messageFile } : null
  } catch (error) {
    throw error
  }
}
