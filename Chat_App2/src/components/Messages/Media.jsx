...
export default function Media({ incoming, author, message, sender }) {
  // Nhận sender từ props
  ...
  // const [deletedByUsers, setDeletedByUser] = useState(
  ...
  // Socket receive revoke/remove (thêm để đồng bộ với Text nếu cần)
  // useEffect(() => {
  //   const handleReceiveRevokeMessage = (newMessage) => {
  //     if (
  //       newMessage?.conversation_id === activeConversation?.id &&
  //       newMessage?.id === message?.id
  //     ) {
  //       setIsRevoked(newMessage?.is_revoked);
  //     }
  //   };
  ...
  // useEffect(() => {
  //   const handleReceiveRemovedMessage = (newMessage) => {
  //     if (
  //       newMessage?.conversation_id === activeConversation?.id &&
  //       newMessage?.id === message?.id
  //     ) {
  //       setDeletedByUser(newMessage?.deleted_by_users);
  //     }
  //   };
  ...
  return incoming && message?.files?.length > 0 ? (
    <div className='max-w-125 w-fit'>
      <div className='flex items-center gap-4'>
        ...
        >
          {!message?.is_revoked && !isDeleted ? (
            <>
              <MediaMsgGrid incoming={incoming} assets={message?.files} />
            ...
            <p className='text-white italic'>
              {message.sender_id === user?.id
                ...
                : `${sender?.display_name || "Someone"} revoked this message`}
            </p>
          ) : (
            isDeleted && (
              ...
              <span className='w-full flex justify-end relative gap-2'>
                ...
                {showReactionBar && (
                  <div className='absolute bottom-0 right-[-100%] translate-x-[100%] z-[100000000]'>
                    <ReactionsPicker handlePickReaction={handlePickReaction} />
                  </div>
                )}
              </span>
            </span>
      ...
      <div className='flex items-center gap-4'>
        ...
        >
          {!message?.is_revoked && !isDeleted ? (
            <>
              <MediaMsgGrid incoming={incoming} assets={message?.files} />
              <p className='text-left text-white'>{message.content}</p>
              <div className='absolute bottom-[-1%] right-0'>
                <ReactionModal
                  handlePickReaction={handlePickReaction}
                  selectedReactions={selectedReactions}
